...
def reach_attack(data, model, args, lb, ub):
    ...
    if args.dataset in ['cora', 'citeseer', 'pubmed']:
        cand_nodes = sorted_indices[degree[sorted_indices] < bar]
    else:
        cand_nodes = sorted_indices[degree[sorted_indices] < bar][:pert_nodes * 5]
    # cand_nodes = sorted_indices[degree[sorted_indices] < bar][:pert_nodes * 10].tolist()
    
    cand_nodes = cand_nodes[torch.randperm(len(cand_nodes))].tolist()
...
if args.dataset in ['reddit']:
    gat = train_model.GAT(data.num_features, 32,
        ...
        4, args.dropout).to(device)
    gat.load_state_dict(torch.load('reddit_models/reddit'+str(args.seed)+'.pt'))
    gat.eval()
    test_acc_gat  = evaluate(gat.cpu(), data.cpu(), data.split_idx['test'].cpu())
else:
...
if args.dataset in ['reddit']:
    test_acc_gat_attack = evaluate(gat.cpu(), data.cpu(), data.split_idx['test'].cpu())
else:
