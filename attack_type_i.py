...
def evaluate(model, data, mask, target_label):
    ...
    with torch.no_grad():
        ...
        acc_target = sum(indices[bools] == target_label) / sum(bools)
        return acc_all, acc_target
...
def reach_attack(data, model, args, lb, ub, target_label):
    ...
    for indx in cand_nodes:
        ...
        adj_train_bools = torch.logical_and(classes[adj_indices]==target_label,
        ...
        adj_non_train_bools = torch.logical_and(torch.logical_not(train_bools[adj_indices]), 
                                                classes_pred[adj_indices] == target_label)
...
while True:
    ...
    savefile_acc = savepath + 'accuracy_attack_target_' + 'data_' + args.dataset + \
    ...
    # y_sgc = sgc(data.x, data.adj_t)
    # preds_sgc = torch.argmax(y_sgc, dim=1)
    # classes_ture = torch.squeeze(data.y)
    # affected_nodes = torch.cat(target_m_idxes)
    ...
    print("{:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}\n".format( \
